...
class admin_model extends Model
{
   ...
   //add veterinarian to database
	...
	public function getDataDashboard(){
		...
		$activeCases=(($this->db->runQuery("SELECT COUNT(CASE WHEN reporttype='Crop Damages' THEN 1 END) AS numOfActiveCropDamage,COUNT(CASE WHEN reporttype='Other Wild Animals are in The Village' THEN 1 END) AS numOfActiveAnimalInVillage,COUNT(CASE WHEN reporttype='Breakdown of Elephant Fences' THEN 1 END) AS numOfActiveBreakdownFence,COUNT(CASE WHEN reporttype='Wild Animal is in Danger' THEN 1 END) AS numOfActiveAnimalIsDanger,COUNT(CASE WHEN reporttype='Illegal Happing' THEN 1 END) AS numOfActiveIllegal,COUNT(CASE WHEN reporttype='Elephants are in The Village' THEN 1 END) AS numOfActiveElephantsVillage FROM reported_incident WHERE status='pending'"))[0]);
		//get  number of success cases of reported incident categories
		$successCases=(($this->db->runQuery("SELECT COUNT(CASE WHEN reporttype='Crop Damages' THEN 1 END) AS numOfActiveCropDamage,COUNT(CASE WHEN reporttype='Other Wild Animals are in The Village' THEN 1 END) AS numOfActiveAnimalInVillage,COUNT(CASE WHEN reporttype='Breakdown of Elephant Fences' THEN 1 END) AS numOfActiveBreakdownFence,COUNT(CASE WHEN reporttype='Wild Animal is in Danger' THEN 1 END) AS numOfActiveAnimalIsDanger,COUNT(CASE WHEN reporttype='Illegal Happing' THEN 1 END) AS numOfActiveIllegal,COUNT(CASE WHEN reporttype='Elephants are in The Village' THEN 1 END) AS numOfActiveElephantsVillage FROM reported_incident WHERE status='success'"))[0]);
		//get  number of unsuccess cases of reported incident categories
		$unSuccessCases=(($this->db->runQuery("SELECT COUNT(CASE WHEN reporttype='Crop Damages' THEN 1 END) AS numOfActiveCropDamage,COUNT(CASE WHEN reporttype='Other Wild Animals are in The Village' THEN 1 END) AS numOfActiveAnimalInVillage,COUNT(CASE WHEN reporttype='Breakdown of Elephant Fences' THEN 1 END) AS numOfActiveBreakdownFence,COUNT(CASE WHEN reporttype='Wild Animal is in Danger' THEN 1 END) AS numOfActiveAnimalIsDanger,COUNT(CASE WHEN reporttype='Illegal Happing' THEN 1 END) AS numOfActiveIllegal,COUNT(CASE WHEN reporttype='Elephants are in The Village' THEN 1 END) AS numOfActiveElephantsVillage FROM reported_incident WHERE status='unsuccess'"))[0]);
	...
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='pending' AND reported_incident.reporttype='Elephants are in The Village'");
	}
	public function successElephantAttack()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='success' AND reported_incident.reporttype='Elephants are in The Village'");
	}
	public function unsuccessElephantAttack()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='unsuccess' AND reported_incident.reporttype='Elephants are in The Village'");
	}
	...
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='pending' AND reported_incident.reporttype='Other Wild Animals are in The Village'");
	}
	public function successAnimalsInVillage()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='success' AND reported_incident.reporttype='Other Wild Animals are in The Village'");
	}
	public function unsuccessAnimalsInVillage()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='unsuccess' AND reported_incident.reporttype='Other Wild Animals are in The Village'");
	}


	public function activeAnimalIsInDanger()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='pending' AND reported_incident.reporttype='Wild Animal is in Danger'");
	}
	public function successAnimalIsInDanger()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='success' AND reported_incident.reporttype='Wild Animal is in Danger'");
	}
	public function unsuccessAnimalIsInDanger()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='unsuccess' AND reported_incident.reporttype='Wild Animal is in Danger'");
	}
	...
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='pending' AND reported_incident.reporttype='Illegal Happing'");
	}

	public function successIllegalActivities()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='success' AND reported_incident.reporttype='Illegal Happing'");
	}
	public function unsuccessIllegalActivities()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='unsuccess' AND reported_incident.reporttype='Illegal Happing'");
	}

	public function activeCropDamages()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='pending' AND reported_incident.reporttype='Crop Damages'");
	}
	public function successCropDamages()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='success' AND reported_incident.reporttype='Crop Damages'");
	}
	public function unsuccessCropDamages()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='unsuccess' AND reported_incident.reporttype='Crop Damages'");
	}

	public function activeFenceDamages()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='pending' AND reported_incident.reporttype='Breakdown of Elephant Fences'"); 
	}
	public function successFenceDamages()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='success' AND reported_incident.reporttype='Breakdown of Elephant Fences'"); 
	}
	public function unsuccessFenceDamages()
	{
		return $this->db->runQuery("SELECT DISTINCT reported_incident.incidentID AS incidentID,village.name AS village,reported_incident.villager_NIC AS NIC,user.Fname AS name,reported_incident.date AS date,reported_incident.time_IN AS time,reported_incident.status AS status,reported_incident.lat AS lat,reported_incident.lon AS lon FROM reported_incident,user,village WHERE user.NIC=reported_incident.villager_NIC AND village.village_code=reported_incident.village_code AND reported_incident.status='unsuccess' AND reported_incident.reporttype='Breakdown of Elephant Fences'"); 
	}
